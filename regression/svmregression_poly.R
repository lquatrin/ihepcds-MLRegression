...
if(require(e1071)){
  #r_svr_e1071_pol <- svm(formula = Y ~ X , data = dataset_test, cost = 1, kernel = "radial")
  r_svr_e1071_pol <- svm(Y ~ X , dataset_test)
  ...
  r_svr_e1071_pol_tuned <- svr_tuneResult$best.model
  ...
  title(main="SVM Polynomial Regression: Training")

  #Second plot
  plot(x <- dataset$X, y <- dataset$Y, pch=16, xlim = c(0,1431), xlab="Day", ylab="minute-average active power")
  lines(dataset$X, predict(r_svr_e1071_pol_tuned, dataset), col = "red", pch=4)
