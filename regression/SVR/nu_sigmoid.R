...
if(require(e1071)){
  r_e1071_svr_nu_sig <- svm(formula = Y ~ X ,
                         type = "nu-regression",
  ...
  predictedY <- predict(r_e1071_svr_nu_sig, dataset_training)
  ...
  predictedY <- predict(r_e1071_svr_nu_sig, dataset_test)
  ...
  lines(dataset$X, predict(r_e1071_svr_nu_sig, dataset), col = "red", pch=4)
}
