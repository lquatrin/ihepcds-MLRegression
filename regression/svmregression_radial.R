...
if(require(e1071)){
  ...
  r_svr_e1071_rad <- svm(formula = Y ~ X , type = "eps-regression", data = dataset_test, gamma = 1, epsilon = 0.1, cost = 1, kernel = "radial")
  #r_svr_e1071_rad <- svm(Y ~ X , dataset_test)
  ...
  r_svr_e1071_rad_tuned <- svr_tuneResult$best.model
  ...
  title(main="SVM Radial Regression: Training")

  #Second plot
  plot(x <- dataset$X, y <- dataset$Y, pch=16, xlim = c(0,1431), xlab="Day", ylab="minute-average active power")
  lines(dataset$X, predict(r_svr_e1071_rad_tuned, dataset), col = "red", pch=4)
